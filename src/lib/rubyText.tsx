...
function LawRefLink({ text, lawTitle }: { text: string; lawTitle: string }) {
  ...
  const [popupInfo, setPopupInfo] = useState<{ lawId: string; title: string; lawNum: string } | null>(null)
  ...
  const handleClick = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (target) {
        setPopupInfo({ lawId: target.law_info.law_id, title: target.revision_info.law_title, lawNum: target.law_info.law_num })
      }
    } catch {
      // ignore
    } finally {
  ...
  return (
    <span className="relative inline-block">
      ...
      {popupInfo && (
        <LawPopup
          lawId={popupInfo.lawId}
          lawTitle={popupInfo.title}
          lawNum={popupInfo.lawNum}
